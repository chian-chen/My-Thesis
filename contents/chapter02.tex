...
If we use a fixed number of bits for every symbol, we waste space. \textbf{Variable-Length Coding (VLC)} solves this by assigning short codes to frequent symbols and long codes to rare ones. Classical methods like Huffman coding~\cite{huffman} use this principle to approach the theoretical entropy limit. Modern standards typically use Arithmetic Coding, which is discussed later in Section~\ref{subsec:entropy_coding}.

\noindent \textbf{Color Space Representation}

Videos are captured in RGB, but the human eye is more sensitive to brightness than color. To save data, we convert RGB into the \textbf{YUV} color space. The Luminance ($Y$) keeps the details, while the Chrominance ($U, V$) is often reduced in resolution (e.g., 4:2:0 subsampling). This reduces the file size with almost no visible loss in quality.
% \noindent \textbf{Coding Redundancy}
% \label{subsec:coding_redundancy}
...
\noindent \textbf{Quantization and Reconstruction}

In both hybrid frameworks and Neural Video Coding (NVC), \textbf{Quantization} serves as the decisive mechanism to govern this trade-off. It maps high-precision transform coefficients (or latent features) to a finite set of discrete levels, thereby reducing information entropy. 
...
Figure \ref{fig:quant_matrix} illustrates this concept using a block-based quantization matrix, common in traditional standards. The process typically involves division by a step size (or matrix element), followed by a rounding operation.
The figure demonstrates the impact of quantization granularity:
\begin{itemize}
    \item \textbf{High Quality Path (Top):} Using fine quantization steps preserves details. The reconstruction is nearly lossless (e.g., the value is restored as $125$), but the result retains high entropy.
    \item \textbf{Low Bitrate Path (Bottom):} Using coarse quantization forces many small values to zero. While this sparsity significantly reduces the bitrate, it introduces irreversible error (e.g., the value becomes $120$).
\end{itemize}
...
\begin{figure}[!h]
    \centering
    \includegraphics[width=0.8\columnwidth]{imgs/rd_curve.pdf}
...
Traditional codecs split the image into blocks and assume objects move rigidly. This often fails for complex motions (like deformation). NVC solves this by using \textbf{Optical Flow}, which estimates motion for every single pixel.

\noindent \textbf{Pyramid-based Motion Estimation}

DVC uses a network called SPyNet~\cite{35} (see Fig. \ref{fig:spynet}). It uses a "pyramid" structure to handle both large and small movements efficiently. Through joint training, the network learns to generate motion fields that are accurate but also easy to compress.

\noindent \textbf{Motion Compensation and Compression}

After warping the frame with optical flow, a \textbf{Motion Compensation Network} refines the result to remove artifacts. Since pixel-wise optical flow is very large, it must be compressed. DVC uses an \textbf{MV Encoder-Decoder} (Fig. \ref{fig:mv_codec}) to compress the motion field into a small latent representation.
...
\begin{figure}[!h]
    ...
    \begin{subfigure}[b]{0.48\textwidth}
        \centering
        \includegraphics[width=\textwidth]{imgs/SpyNet.png}
        \caption{Optical Flow Estimation (SPyNet)~\cite{35}.}
        \label{fig:spynet}
    \end{subfigure}
    \hfill
    \begin{subfigure}[b]{0.48\textwidth}
        \centering
        \includegraphics[width=\textwidth]{imgs/MV_Encoder.png}
        \caption{MV Encoder and Decoder Network~\cite{17}.}
        \label{fig:mv_codec}
    \end{subfigure}
    \caption{The motion processing pipeline in DVC.}
    \label{fig:motion_processing}
...
% \subsubsection*{Limitations of Linear Transforms}
...
Traditional standards use the Discrete Cosine Transform (DCT)~\cite{wiegand2003using}. DCT is linear and processes images in fixed $8\times8$ blocks. This can cause "blocking artifacts" and cannot capture complex structures that cross block boundaries.

NVC replaces this with \textbf{Learned Non-linear Transforms} (CNNs with GDN layers~\cite{65}). As shown in Fig. \ref{fig:transform_comparison}, the learned features (c) are not limited to blocks. They automatically focus on important structures like object boundaries. This allows the system to allocate bits more intelligently based on visual importance.

\begin{figure}[!h]
    \centering
    \includegraphics[width=1\textwidth]{imgs/transform.pdf}
    \caption{Visual comparison of spatial energy distribution. The NVC model~\cite{63} extracts semantic features (c) using \textbf{CompressAI}~\cite{32}, unlike the rigid blocks in traditional DCT (b).}
    \label{fig:transform_comparison}
\end{figure}
...
\subsubsection*{Learned Entropy Models}
